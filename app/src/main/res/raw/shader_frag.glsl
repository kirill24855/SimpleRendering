...
void main() {
	...
	for (int i = 0; i < maxIteration; i++) {
		...
		x2 = ds_mul(vec2(tz.x, tz.z), vec2(tz.x, tz.z));
		y2 = ds_mul(vec2(tz.y, tz.w), vec2(tz.y, tz.w));

		vec2 zx = ds_sub(x2, y2);
		zx = ds_add(zx, vec2(uv.x, uv.z));

		vec2 zy = ds_mul(vec2(tz.x, tz.z), vec2(tz.y, tz.w));
		zy = ds_mul(zy, vec2(2.0, 0.0));
		zy = ds_add(zy, vec2(uv.y, uv.w));
